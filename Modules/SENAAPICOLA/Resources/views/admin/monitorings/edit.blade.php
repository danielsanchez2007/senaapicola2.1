            ...
            <div class="col-12 col-lg-10 col-xl-8">
                <div class="form-card">
                    ...
                    <div class="card-header">
                        ...
                        <h3 class="fw-bold mb-2">Editar Seguimiento</h3>
                        <p class="opacity-80 small mb-0">
                            Actualiza la información del seguimiento de la colmena / apiario
                        </p>
                    </div>
                    ...
                    <div class="card-body p-5">
                        <form action="{{ route('senaapicola.admin.monitorings.update', $monitoring->id) }}" method="POST">
                            ...
                            <div class="row g-4">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="date" name="date" class="form-control" 
                                           value="{{ $monitoring->date }}" required>
                                </div>
                                    ...
                                    <select name="apiary_id" class="form-select" required>
                                        ...
                                        @foreach($apiaries as $apiary)
                                            <option value="{{ $apiary->id }}" 
                                                {{ $apiary->id == $monitoring->apiary_id ? 'selected' : '' }}>
                                                {{ $apiary->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select id="user_nickname" name="user_nickname" class="form-select" required>
                                        ...
                                        <option value="{{ $user->nickname }}" 
                                                data-role="{{ $roleSlug }}" 
                                                {{ $user->nickname == $monitoring->user_nickname ? 'selected' : '' }}>
                                            {{ $user->nickname }}
                                        </option>
                                        @endif
                                        @endforeach
                                ...
                                <!-- Rol (automático) -->
                                <div class="col-md-6">
                                    <label class="form-label">Rol del Responsable</label>
                                    @php
                                        $readableRole = $monitoring->role === 'senaapicola.admin' ? 'Administrador' :
                                        ($monitoring->role === 'senaapicola.intern' ? 'Pasante' : $monitoring->role);
                                    @endphp
                                    <input type="text" id="role" class="form-control" 
                                           value="{{ $readableRole }}" readonly>
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea name="description" rows="4" class="form-control">{{ $monitoring->description }}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="d-flex gap-3 mt-5">
                                ...
                                <a href="{{ route('senaapicola.admin.monitorings.index') }}" 
                    ...
                    <div class="card-footer bg-transparent border-0 text-center py-4 small text-white-50">
                        Los cambios se reflejarán en los reportes y en el historial de la colmena.
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    // Script para actualizar rol automáticamente al cambiar usuario
    document.addEventListener('DOMContentLoaded', function () {
        ...
        userSelect.addEventListener('change', function () {
            ...
            const roleSlug = selectedOption.getAttribute('data-role') || '';
            const readableRole = roleMapping[roleSlug] || '';
            
            roleInput.value = readableRole;
            hiddenRole.value = roleSlug;
        });
    });
</script>
@endpush