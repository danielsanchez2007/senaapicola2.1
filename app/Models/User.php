...
class User extends Authenticatable implements Auditable
{
    ...
    /**
     * Usuario solo con rol informativo (sin panel admin ni pasante).
     */
    public function isSenaApicolaInfoViewer(): bool
    {
        $slugs = $this->roles()->pluck('slug')->all();

        $hasInfo = in_array('senaapicola.usuarioinfo', $slugs, true);
        $hasApp = in_array('senaapicola.admin', $slugs, true)
            || in_array('senaapicola.intern', $slugs, true);

        return $hasInfo && ! $hasApp;
    }
